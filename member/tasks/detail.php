...
";
$stmt = sqlsrv_query($conn, $sql, [$id, (int)$user['id']]);
...
$updateStmt = sqlsrv_query(
    $conn,
    "SELECT progress_percent, note, created_at
     FROM task_updates
...
$submissionStmt = sqlsrv_query(
    ...
    "SELECT s.*
     FROM submissions s
     WHERE s.task_id = ? AND s.submitted_by = ?
    ...
    <main class="page-content">
        ...
        <div class="app-page-head d-flex justify-content-between align-items-center mb-4">
            <div>
                ...
                <p class="text-muted mb-0">Thông tin chi tiết công việc của bạn</p>
            </div>
            <div class="d-flex gap-2">
                <?php if ($task['status'] !== 'completed' && $task['status'] !== 'submitted'): ?>
                    <a href="<?= e(base_url('/member/tasks/update_progress.php?id=' . $task['id'])) ?>" class="btn btn-primary">
                        Cập nhật tiến độ
                    </a>
                <?php endif; ?>

                <?php if ($task['status'] !== 'completed'): ?>
                    <a href="<?= e(base_url('/member/tasks/submit_file.php?id=' . $task['id'])) ?>" class="btn btn-success">
                        Nộp file
                    </a>
                <?php endif; ?>

                <a href="<?= e(base_url('/member/tasks/index.php')) ?>" class="btn btn-outline-secondary">
                    Quay lại
                </a>
            </div>
        </div>
        ...
        <div class="row g-4">
            <div class="col-lg-5">
                <div class="card section-card">
                    <div class="card-body p-4">
                        ...
                        <p><strong>Leader giao việc:</strong> <?= e($task['leader_name']) ?></p>
                        <p>
                        ...
                        <p><strong>Tiến độ hiện tại:</strong> <?= e($task['progress_percent']) ?>%</p>
            ...
            <div class="col-lg-7">
                <div class="card section-card mb-4">
                    ...
                    <div class="card-body px-4">
                        <?php if (empty($updates)): ?>
                            <p class="text-muted mb-0">Bạn chưa cập nhật tiến độ lần nào.</p>
                        <?php else: ?>
                            <div class="list-group list-group-flush">
                                <?php foreach ($updates as $item): ?>
                                    <div class="list-group-item px-0">
                                        <div class="d-flex justify-content-between">
                                            <div class="fw-semibold"><?= e($item['progress_percent']) ?>%</div>
                ...
                <div class="card section-card">
                    <div class="card-header bg-white border-0 pt-4 px-4">
                        <h5 class="mb-0 fw-bold">Bài nộp của bạn</h5>
                    </div>
                    <div class="card-body px-4">
                        <?php if (empty($submissions)): ?>
                            <p class="text-muted mb-0">Bạn chưa nộp file nào cho công việc này.</p>
                        <?php else: ?>
                ...
                <div class="card section-card mt-4" id="task-comments">
                    ...
                    <div class="card-body px-4">
                        <form method="POST" action="<?= e(base_url('/task_comments/store.php')) ?>" class="mb-4 app-inline-form">
                            ...
                            <input type="hidden" name="redirect_to" value="<?= e(task_detail_url_for_role('member', $id) . '#task-comments') ?>">
                            <label class="form-label">Thêm bình luận</label>
                            <textarea name="comment_text" rows="3" class="form-control" placeholder="Đặt câu hỏi hoặc cập nhật nhanh cho leader" required></textarea>
                            <div class="mt-3">
